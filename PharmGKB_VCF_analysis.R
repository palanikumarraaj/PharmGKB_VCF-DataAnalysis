...
#vc <- read.delim(file = "10588141_184.bwa.vcf", header = TRUE)
vc <- read.delim(seqName, header = TRUE)
sjoined <- left_join(df_clean, vc, by = c("ID"))
...
result_column <- c(
  'Clinical.Annotation.ID', 'Gene', 'Level.of.Evidence',
  'Score', 'Phenotype.Category', 'Drug', 'Phenotype', 'POS',
  'QUAL', 'INFO', 'REF', 'ALT', 'Annotation.Text'
)

# Assign column names to result_data
colnames(result_data) <- result_column
